...
mean = np.mean(inc)
std = np.std(inc)
stdIncome = (inc - mean)/std
plt.figure()
sns.boxplot(data=stdIncome)
plt.show()
```
...
carseats['Urban'] = carseats['Urban'].fillna(carseats['Urban'].mode())
```
...
def drop_by_iqr(data, column):
    ...
    outlier_index = data[(data[column] < lower_bound) | (data[column] > upper_bound)].index
