...
![png](Report_5_files/Report_5_32_0.png)
...
```python
### No, because it contains text (categorical data), not numbers. Only numerical data can be averaged
```
...
sum_of_deviations = sum(deviations)
...
![png](Report_5_files/Report_5_41_0.png)
...
```python
### No, because it contains text (categorical data), not numbers. The median requires values that can be ordered.
```
...
![png](Report_5_files/Report_5_53_0.png)
...
# Interpretation:
- Median shows the central value of the data 
- Q1 and Q3 give the IQR, where 50% of the data lies.
- If the mean is far from the median, plot has skewness.
- Symmetrical violin shape means normal distribution;
...
![png](Report_5_files/Report_5_57_0.png)
...
![png](Report_5_files/Report_5_60_0.png)
...
```python
# Skewness equal 0.6562 means that the distribution is positively skewed (right-skewed).
# Most values are in the left, with some outliers on the right.
# Mean > Median
```
...
fig, axes = plt.subplots(2, 2, figsize=(12, 10))  # Ensure 2 rows and 2 columns

sns.histplot(df_pokemon[df_pokemon['Legendary'] == True]['Attack'], color='green', ax=axes[0, 0])
axes[0, 0].set_title("Legendary Pokemon Attack Histogram")
axes[0, 0].axvline(df_pokemon[df_pokemon['Legendary'] == True]['Attack'].mean(), linestyle="dotted", color="blue")
axes[0, 0].legend(['Mean'])

sns.histplot(df_pokemon[df_pokemon['Legendary'] == False]['Attack'], color='red', ax=axes[0, 1])
axes[0, 1].set_title("Non-Legendary Pokemon Attack Histogram")
axes[0, 1].axvline(df_pokemon[df_pokemon['Legendary'] == False]['Attack'].mean(), linestyle="dotted", color="blue")
axes[0, 1].legend(['Mean'])

sns.boxplot(x=df_pokemon[df_pokemon['Legendary'] == True]['Attack'], color='green', ax=axes[1, 0])
axes[1, 0].set_title("Legendary Pokemon Attack Boxplot")

sns.boxplot(x=df_pokemon[df_pokemon['Legendary'] == False]['Attack'], color='red', ax=axes[1, 1])
axes[1, 1].set_title("Non-Legendary Pokemon Attack Boxplot")
...
![png](Report_5_files/Report_5_102_1.png)
...
```python
# As we can see, Legendary Pokémon have higher attack values than non-Legendary Pokémon.
# This is evident from their higher mean and median, as shown in both the histograms and box plots.
# The box plot also reveals that the interquartile range (IQR) is narrower for Legendary Pokémon, meaning their attack stats are more concentrated around the central values.
# In contrast, non-Legendary Pokémon show greater variability, with wider IQR.
# The positive kurtosis (+0.15) for non-Legendary Pokémon indicates a slightly higher chance for extreme high attack values than would be expected in a normal distribution.
# The negative kurtosis (-0.19) or Legendary Pokemon suggests their attack values are closer to the mean, indicating fewer extreme values than in a normal distribution.
# This also implies that non-Legendary Pokemon are more likely to have outliers in attack stats.
# Both graphs are right-skewed, which means that most Pokémon have lower attack values, but a few have very high ones that stretch the distribution to the right.
```
